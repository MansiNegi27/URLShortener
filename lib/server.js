  ...
  myHandler(sendData,function(statusCode,payload)
{
  ...
  {
    if(statusCode === "homepage")
    {
      //This is the case when the user requests the home page
      res.setHeader('Content-Type','text/html');
      res.setHeader('Access-Control-Allow-Origin','*');
      res.end(payload);
    }else {
      //When the user is trying to acess a shortened url
    res.setHeader('Access-Control-Allow-Origin','*');
    res.writeHead(302, {'location':'https://'+payload.userUrl});
    res.end();
    }
  }else {
    //This is for the case when the user wants to shorten a url
